...
feature_info_plot <- function(var, ...) {
  ...
  top_track <- spotify |>
    ...
    slice(1) |>
    select(track_name, artists, !!var := .data[[var]])
  
  # Count missing values for the feature
  n_missing <- spotify |>
    summarize(missing = sum(is.na(.data[[var]]))) |>
    pull(missing)
  ...
  # Return a list of summary info and plot
...
- `.data[[var]]` lets us dynamically reference column names inside `dplyr` and `ggplot2`, making the function reusable.  
...
weather_check |>
  ...
  mutate(
    ...
    text = paste0(weather_source, "<br>", n, " responses (", pct, ")")  # Custom hover text
  ) |>
  plot_ly(
